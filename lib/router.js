...
Router:
  stack: Array[Layer]
  params: Map[RouterName, Middleware]
  opts: Map[String, Any] 
    sensitive: Boolean,
    strict: Boolean,
    prefix: String
    methods:Array[String] // default allowed methods
    routerPath
     
  matched
  method

  methods: Array[String] default to {'HEAD', 'OPTIONS', 'GET', 'PUT', 'PATCH', 'POST', 'DELETE'}// 
...
methods.forEach(function (method) {//meta programming - using code to generate code
...
Router.prototype.use = function () {
  ...
  middleware = middleware.filter(function (fn) {
    if (fn.router) {// setted by .routes method
      fn.router.stack.forEach(function (layer) {
        ...
        router.stack.push(layer);//extract nested layers out
      });

      if (router.params) {//todo:
 ...
 *
 * @returns {Function} GeneratorFunc
 */
...
Router.prototype.routes = Router.prototype.middleware = function () {
  ...
  var dispatch = function *dispatch(next) {
    ...
    if (this.matched) {// this ref to dispatch function instance
    ...
    if (matched.pathAndMethod.length) {
      ...
      while (matched.route && i--) {//todo: this logic ?
        layer = matched.pathAndMethod[i];
        ii = layer.stack.length;
        this.captures = layer.captures(path, this.captures);// arrays match group of path regex
        this.params = layer.params(path, this.captures, this.params);//return {},
        debug('dispatch %s %s', layer.path, layer.regexp);

        while (ii--) {
          if (layer.stack[ii].constructor.name === 'GeneratorFunction') {
            // bm: use this to pass `capture, params` to the caller
...
/*
throw:Boolean, // wether throw notImplemented() error when condition user requested with invalid method
notImplemented: ()=>error // if user request with invalid method, default to HttpError.NotImplemented()
*/
Router.prototype.allowedMethods = function (options) {
  ...
  return function *allowedMethods(next) {
    ...
    if (!this.status || this.status === 404) {
      ...
      if (!~implemented.indexOf(this.method)) {// true if implemented.indexOf(this.method) === -1
        if (options.throw) {
 ...
 * @param opts: Object
            end, name, sensitive, strict, prefix
 * @returns {Layer}
 * @private
 */
...
Router.prototype.register = function (path, methods, middleware, opts) {
  ...
  Object.keys(this.params).forEach(function (param) {// desc order, last ele inserted on the most top.
  ...
  if (methods.length || !stack.length) {
    ...
    if (!route.paramNames.length) {// this route is not a param route (route contains param pattern)
      ...
      added = stack.some(function (m, i) {
        ...
        if (routeNestingLevel === mNestingLevel && isParamRoute) {// insert right before the same level param route
    ...
    stack.some(function (m, i) {
      if (!m.methods.length && i === stack.length - 1) {
        ...
        if (stack[i - 1]) {// if i is in the middle of this array
          return stack.splice(i, 0, route);
        } else {
...
Router.prototype.match = function (path, method) {
  ...
  var matched = {
    path: [],                  // match paths
    pathAndMethod: [],         // match paths & method
    route: false               // any layer contains specified methods
  };
  ...
  for (var len = layers.length, i = 0; i < len; i++) {
    ...
    if (layer.match(path)) {
      ...
      if (layer.methods.length === 0 || ~layer.methods.indexOf(method)) {// layer.methods.indexOf(method) != -1
